...
export async function up(db: Kysely<unknown>): Promise<void> {
  await createTableWithAutoIncrementId(db as any, 'meta_event_handler')
    .addColumn('created_on', 'timestamp', (col: ColumnDefinitionBuilder) => col.defaultTo(sql`CURRENT_TIMESTAMP`))
    .addColumn('modified_on', 'timestamp', (col: ColumnDefinitionBuilder) => col.defaultTo(sql`CURRENT_TIMESTAMP`))
  ...
  await createTableWithAutoIncrementId(db as any, 'meta_workflow_def')
    ...
    .addColumn('latest_version', 'integer', (col: ColumnDefinitionBuilder) => col.notNull().defaultTo(0))
    .addColumn('json_data', 'text', (col: ColumnDefinitionBuilder) => col.notNull())
  ...
  await createTableWithAutoIncrementId(db as any, 'event_execution')
    .addColumn('created_on', 'timestamp', (col: ColumnDefinitionBuilder) => col.defaultTo(sql`CURRENT_TIMESTAMP`))
    .addColumn('modified_on', 'timestamp', (col: ColumnDefinitionBuilder) => col.defaultTo(sql`CURRENT_TIMESTAMP`))
    .addColumn('event_handler_name', 'varchar(255)', (col: ColumnDefinitionBuilder) => col.notNull())
    ...
    .addColumn('json_data', 'text', (col: ColumnDefinitionBuilder) => col.notNull())
    .addUniqueConstraint('unique_event_execution', ['event_handler_name', 'event_name', 'message_id'])
    .execute();
  ...
  await db.schema.createIndex('poll_data_queue_name_index').on('poll_data').column('queue_name').execute();
  ...
  await createTableWithAutoIncrementId(db as any, 'task_in_progress')
    ...
    .addColumn('task_id', 'varchar(255)', (col: ColumnDefinitionBuilder) => col.notNull())
    .addColumn('workflow_id', 'varchar(255)', (col: ColumnDefinitionBuilder) => col.notNull())
    .addColumn('in_progress_status', 'boolean', (col: ColumnDefinitionBuilder) => col.notNull().defaultTo(false))
  ...
  await createTableWithAutoIncrementId(db as any, 'workflow')
    .addColumn('created_on', 'timestamp', (col: ColumnDefinitionBuilder) => col.defaultTo(sql`CURRENT_TIMESTAMP`))
    .addColumn('modified_on', 'timestamp', (col: ColumnDefinitionBuilder) => col.defaultTo(sql`CURRENT_TIMESTAMP`))
    .addColumn('workflow_id', 'varchar(255)', (col: ColumnDefinitionBuilder) => col.notNull().unique())
  ...
  await createTableWithAutoIncrementId(db as any, 'workflow_def_to_workflow')
    ...
    .addColumn('workflow_id', 'varchar(255)', (col: ColumnDefinitionBuilder) => col.notNull())
    .addUniqueConstraint('unique_workflow_def_date_str', ['workflow_def', 'date_str', 'workflow_id'])
    .execute();
  ...
  await createTableWithAutoIncrementId(db as any, 'workflow_pending')
    ...
    .execute();
  await db.schema.createIndex('workflow_type_index').on('workflow_pending').column('workflow_type').execute();
  ...
  await createTableWithAutoIncrementId(db as any, 'workflow_to_task')
    ...
    .execute();
  await db.schema.createIndex('workflow_id_index').on('workflow_to_task').column('workflow_id').execute();

  await createTableWithAutoIncrementId(db as any, 'queue')
    .addColumn('created_on', 'timestamp', (col: ColumnDefinitionBuilder) => col.defaultTo(sql`CURRENT_TIMESTAMP`))
    .addColumn('queue_name', 'varchar(255)', (col: ColumnDefinitionBuilder) => col.notNull().unique())
    .execute();

  await createTableWithAutoIncrementId(db as any, 'queue_message')
    .addColumn('created_on', 'timestamp', (col: ColumnDefinitionBuilder) => col.notNull().defaultTo(sql`CURRENT_TIMESTAMP`))
    .addColumn('deliver_on', 'timestamp', (col: ColumnDefinitionBuilder) => col.defaultTo(sql`CURRENT_TIMESTAMP`))
    .addColumn('queue_name', 'varchar(255)', (col: ColumnDefinitionBuilder) => col.notNull())
    .addColumn('message_id', 'varchar(255)', (col: ColumnDefinitionBuilder) => col.notNull())
  ...
  await db.schema.createIndex('combo_queue_message').on('queue_message').columns(['queue_name', 'popped', 'deliver_on', 'created_on']).execute();
}
