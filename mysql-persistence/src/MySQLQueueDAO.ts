...
export class MySQLQueueDAO implements QueueDAO {
  ...
  async push(
    ...
    const updateResult = await this.db
      ...
      .set({
        deliver_on: sql`CURRENT_TIMESTAMP + INTERVAL ${offsetTimeInSecond} SECOND`,
      })
      .where('queue_name', '=', queueName)
      .where('message_id', '=', id)
      .executeTakeFirst();
    ...
    if (updateResult.numUpdatedRows === 0n) {
      await this.db
        ...
        .onDuplicateKeyUpdate({
          deliver_on: sql`VALUES(deliver_on)`,
        })
        .execute();
    }
  }
  ...
  async pushIfNotExists(
    ...
    const result = await this.db
      ...
      .onDuplicateKeyUpdate({
        queue_name: sql`VALUES(queue_name)`,
      })
      .executeTakeFirst();
    ...
    const popped = await this.db
      .with('cte', (db) =>
        db
          ...
          .where('queue_name', '=', queueName)
          .where('popped', '=', false)
          .where('deliver_on', '<=', sql<Date>`CURRENT_TIMESTAMP + INTERVAL 1000 MICROSECOND`)
          .orderBy('deliver_on')
  ...
  async setUnackTimeout(
    ...
    const result = await this.db
      ...
      .set({
        ...
        deliver_on: sql`CURRENT_TIMESTAMP + INTERVAL ${updatedOffsetTimeInSecond} SECOND`,
      })
      .where('queue_name', '=', queueName)
      .where('message_id', '=', messageId)
      .executeTakeFirst();
  ...
  async setUnackTimeoutIfShorter(
    ...
    const result = await this.db
      ...
      .where(
        'deliver_on',
        '>',
        sql<Date>`CURRENT_TIMESTAMP + INTERVAL ${updatedOffsetTimeInSecond} SECOND`,
      )
      .executeTakeFirst();
    return result.numUpdatedRows === 1n;
  }
  ...
  async processUnacks(queueName: string): Promise<void> {
    await this.db
      ...
      .where('queue_name', '=', queueName)
      .where('popped', '=', true)
      .where('deliver_on', '<', sql<Date>`CURRENT_TIMESTAMP - INTERVAL 60 SECOND`)
      .execute();
  }
  ...
  async postpone(
    ...
    const result = await this.db
      ...
      .set({
        priority,
        deliver_on: sql`CURRENT_TIMESTAMP + INTERVAL ${postponeDurationInSeconds} SECOND`,
      })
      .where('queue_name', '=', queueName)
      .where('message_id', '=', messageId)
      .executeTakeFirst();
