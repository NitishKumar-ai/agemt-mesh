...
function addEvent(event) {
    ...
    const typeClass = event.eventType.includes('fail') || event.eventType.includes('timeout') ? 'status-failed' : 
                      event.eventType.includes('system') || event.eventType.includes('killswitch') ? 'status-system' : 'status-running';
...
fetch('/api/agents/killswitch').then(res => res.json()).then(data => {
