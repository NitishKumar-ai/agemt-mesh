...
AgentMesh supports three levels of agent dynamism, from simple tool use to fully self-generating agents.


## Agent loop: plan/act/observe with DO_WHILE

The defining pattern of an autonomous agent is the loop: call an LLM, execute a tool, observe the result, decide whether to continue. AgentMesh models this with `DO_WHILE`:
...
AgentMesh supports dynamic workflow execution where the complete workflow definition is provided at start time, without pre-registration. This is the most powerful form of agent dynamism — the LLM generates the entire execution plan as JSON, and AgentMesh runs it immediately.
...
3. AgentMesh validates, persists, and executes it immediately.
  ...
  "tasks": [
    {
      ...
      "inputParameters": {
        ...
        "messages": [
          {
            "role": "system",
            "message": "You are a workflow planner. Given a user task, generate a AgentMesh workflow definition as JSON. Available task types: LLM_CHAT_COMPLETE, CALL_MCP_TOOL, LIST_MCP_TOOLS, HTTP, HUMAN, LLM_SEARCH_INDEX. The workflow must include a 'name', 'tasks' array, and 'outputParameters'."
          },
          {
            "role": "user",
            "message": "${workflow.input.task}"
...
3. `execution` &mdash; `START_WORKFLOW` launches the generated workflow definition directly. AgentMesh validates it, persists it, and executes it with full durability. No pre-registration needed.

The generated child workflow gets all the same guarantees as any AgentMesh workflow: persisted state, retry policies, failure handling, full observability. The fact that it was generated by an LLM 30 seconds ago doesn't matter — it runs on the same durable execution engine.
...
Every task type here — `LIST_MCP_TOOLS`, `LLM_CHAT_COMPLETE`, `CALL_MCP_TOOL`, `HUMAN` — is a native AgentMesh system task. No custom workers, no external frameworks.

See the full set of examples in the [`ai/examples/`](https://github.com/agentmesh-oss/agentmesh/tree/main/ai/examples) directory.
