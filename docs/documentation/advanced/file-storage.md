...
```properties
agentmesh.file-storage.enabled=true
```
...
Implement `org.agentmeshoss.agentmesh.core.storage.FileStorage` and register it as a Spring bean. The default service uses the bean that matches `agentmesh.file-storage.type`.
...
Objects are written to `agentmesh/<workflowId>/<fileId>` inside the configured bucket / container / directory. Layout is the same across backends.
...
Workers consume and produce files via the Java SDK's `FileHandler` type. See the [File handling](../clientsdks/java-sdk.md#file-handling) section of the Java SDK page and the [Media Transcoder](https://github.com/agentmesh-oss/file-storage-java-sdk/tree/main/examples/file-storage/media-transcoder) example.
