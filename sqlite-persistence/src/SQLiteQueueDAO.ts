...
export class SqliteQueueDAO implements QueueDAO {
    ...
    const popped = await this.db
      .with('cte', (db) =>
        db
          ...
          .where('queue_name', '=', queueName)
          .where('popped', '=', sql<boolean>`0`)
          .where('deliver_on', '<=', sql<Date>`datetime('now')`)
          .orderBy('deliver_on')
  ...
  async setUnackTimeout(
    ...
    const result = await this.db
      ...
      .set({
        ...
        deliver_on: sql`datetime('now', '+' || ${updatedOffsetTimeInSecond} || ' seconds')`,
      })
      .where('queue_name', '=', queueName)
      .where('message_id', '=', messageId)
      .executeTakeFirst();
  ...
  async setUnackTimeoutIfShorter(
    ...
    const result = await this.db
      ...
      .where(
        'deliver_on',
        '>',
        sql<Date>`datetime('now', '+' || ${updatedOffsetTimeInSecond} || ' seconds')`,
      )
      .executeTakeFirst();
    return result.numUpdatedRows === 1n;
  }
  ...
  async processUnacks(queueName: string): Promise<void> {
    await this.db
      .updateTable('queue_message')
      .set({ popped: sql<boolean>`0` })
      .where('queue_name', '=', queueName)
      .where('popped', '=', sql<boolean>`1`)
      .where('deliver_on', '<', sql<Date>`datetime('now', '-60 seconds')`)
      .execute();
  }
