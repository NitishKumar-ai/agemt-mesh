...
export class CassandraPollDataDAO extends CassandraBaseDAO implements PollDataDAO {
  ...
  async updateLastPollData(taskDefName: string, domain: string, workerId: string): Promise<void> {
    const query =
      'UPDATE poll_data SET last_poll_time = ?, worker_id = ? WHERE queue_name = ? AND domain = ?';
    await this.client.execute(query, [Date.now(), workerId, taskDefName, domain ?? 'DEFAULT'], {
      prepare: true,
    });
  }
  ...
  async getPollData(taskDefName: string, domain: string): Promise<PollData | undefined> {
    ...
    const result = await this.client.execute(query, [taskDefName, domain ?? 'DEFAULT'], {
      prepare: true,
    });
