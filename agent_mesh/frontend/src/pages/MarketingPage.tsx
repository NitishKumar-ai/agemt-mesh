...
export function MarketingPage() {
  ...
  async function create(event: FormEvent) {
    ...
    if (
      !form.source_finding_id ||
      !form.name.trim() ||
      !form.audience.trim() ||
      !form.finding_summary.trim()
    )
      return;
    ...
    <div className="page marketing-page">
      ...
      <section className="finding-inbox">
        <div className="finding-inbox-head">
          <div>
            <span>CommitGuard handoff</span>
            <h2>Security findings</h2>
            <p>Only verified findings can become campaign evidence.</p>
          </div>
          <strong>
            {findings.filter((finding) => finding.status === 'verified').length} verified
          </strong>
        </div>
        <div className="finding-list">
          {findings.length === 0 && (
            <div className="empty-card">No findings have been handed off from CommitGuard yet.</div>
          )}
              ...
              <div className="finding-actions">
                {finding.status === 'review_required' ? (
                  <button
                    ...
                    onClick={() => void verifyFinding(finding.id)}
                  >
                    <ShieldCheck size={14} />
                    Verify
                  </button>
                ) : (
                  <button className="primary-button" onClick={() => useFinding(finding)}>
                    <Megaphone size={14} />
                    Use finding
                  </button>
                )}
      ...
      <div className="marketing-layout">
        <section className="campaign-list">
          {campaigns.length === 0 && (
            <div className="empty-card">No campaigns yet. Create one from a verified finding.</div>
          )}
          {campaigns.map((campaign) => (
            <article className="campaign-card" key={campaign.id}>
              <header>
                ...
                <strong className={`campaign-status campaign-status--${campaign.status}`}>
              ...
              <footer>
                {pipelines[campaign.id] ? (
                  ...
                  <button
                    ...
                    onClick={() => void generate(campaign.id)}
                  >
                    <Sparkles size={14} />
                    {campaign.body ? 'Regenerate draft' : 'Generate draft'}
                  </button>
                )}
                ...
                {campaign.status === 'approved' && (
                  <button
                    className="primary-button"
                    disabled={busy === campaign.id}
                    onClick={() => void schedule(campaign.id)}
                  >
          ...
          <form onSubmit={create}>
            {form.source_finding_id && (
              <div className="selected-finding">
                <ShieldCheck size={14} />
                Using verified finding #{form.source_finding_id}
              </div>
            )}
            <label>
              Campaign name
              <input
                ...
                placeholder="Dependency risk outreach"
              />
            </label>
            <label>
              Audience
              <input
                ...
                placeholder="Engineering leaders at SaaS teams"
              />
            </label>
            <label>
              Verified finding
              <textarea
                ...
                placeholder="Select a verified finding from the inbox above."
              />
            </label>
            <label>
              Value proposition
              <textarea
                ...
                onChange={(event) => setForm({ ...form, value_proposition: event.target.value })}
                placeholder="How your service helps remediate the finding..."
              />
            </label>
            <label>
              Channel
            ...
            <button className="primary-button" disabled={!form.source_finding_id} type="submit">
              <Sparkles size={14} />
              Create campaign
            </button>
          </form>
      ...
      <section className="marketing-audit">
        <div className="marketing-audit-head">
          <History size={16} />
          <div>
            <h2>Audit trail</h2>
            <p>Append-only record of the marketing handoff and approval lifecycle.</p>
          </div>
        </div>
        <div className="audit-event-list">
          {auditEvents.length === 0 && (
            <div className="empty-card">No marketing audit events yet.</div>
          )}
