      ...
      <section>
        ...
        >
          ...
          {STATUS_FILTERS.map((f) => (
            <button
              key={f}
              style={filterChipStyle(statusFilter === f)}
        ...
        <div className="table-card">
          ...
          {filtered.map((s) => (
            <article
              ...
              style={{
                cursor: 'pointer',
                background: selected === s.run_id ? 'var(--surface-card)' : undefined,
              }}
      ...
      <aside className="form-panel" style={{ overflowY: 'auto' }}>
        {!selected && (
          <div style={{ color: 'var(--muted)', fontSize: 14, marginTop: 24 }}>
            <Files
              size={28}
              style={{ marginBottom: 10, display: 'block', color: 'var(--muted-soft)' }}
            />
        ...
        {selected && (
          <>
            ...
            {!loadingSteps && steps.length === 0 && (
              <div className="empty-card">No steps recorded.</div>
            )}
