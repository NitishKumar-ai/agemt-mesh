        ...
        <div className="table-card">
          {workflows.length === 0 && (
            <div className="empty-card">
              No workflow runs yet. Start a session to generate runs.
            </div>
          )}
          {workflows.map((w) => (
            <article className="schedule-row" key={w.run_id}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 4 }}>
                  {w.agent_id} · {w.step_count} step{w.step_count !== 1 ? 's' : ''} · last:{' '}
                  {w.last_step}
                </p>
          ...
          {dlq.map((d) => (
            <article className="task-row" key={d.id}>
              <div>
                ...
                <h3>{d.agent_id}</h3>
                <p style={{ color: 'var(--error)', fontFamily: 'monospace', fontSize: 12 }}>
                  {d.error}
                </p>
