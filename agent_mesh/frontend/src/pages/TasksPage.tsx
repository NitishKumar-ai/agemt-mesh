...
export function TasksPage() {
  ...
  const filtered = tasks.filter((task) => `${task.file_path} ${task.comment} ${task.marker}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        actions={<button className="primary-button" onClick={scan}><RefreshCw size={15} />Scan codebase</button>}
      />
      ...
      <div className="task-list">
        ...
        {!loading && filtered.length === 0 && <div className="empty-card">No tasks found. Run a scan to populate this queue.</div>}
        {filtered.map((task) => (
          <article className="task-row" key={task.id}>
            <div>
              <div className="task-row-meta">
                <span>{task.marker}</span>
                <code>{task.file_path}:{task.line_number}</code>
              </div>
            ...
            <div className="task-row-side">
              ...
              <button className="secondary-button" onClick={() => run(task.id)}><Play size={14} />Run</button>
            </div>
          </article>
        ))}
      </div>
