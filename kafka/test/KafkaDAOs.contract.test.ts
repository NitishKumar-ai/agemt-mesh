...
describe('Kafka DAOs Integration', () => {
  ...
  beforeAll(async () => {
    ...
    execSync(
      ...
      '-e KAFKA_LISTENER_SECURITY_PROTOCOL_MAP=PLAINTEXT:PLAINTEXT,CONTROLLER:CONTROLLER ' +
    ...
    for (let i = 0; i < 60; i++) {
      try {
        const admin = new Kafka({
          clientId: 'test-probe', brokers: ['localhost:9092'],
    ...
    queue = new KafkaQueue({ brokers: ['localhost:9092'], groupId: 'agentmesh-test' });
    await queue.connect();
  }, 180000);
  ...
  const cleanup = async () => {
    const detail = await queue.queuesDetail();
    for (const q of Object.keys(detail)) {
      try { await queue.flush(q); } catch { /* ignore */ }
    }
  };
