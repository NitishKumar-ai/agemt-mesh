...
export class GongConnector implements Connector {
  ...
  constructor(
    ...
    if (!config.identifier) {
      throw new Error('gong connector requires an identifier (User ID) in config to use Scalekit');
    }
  }

  async bootstrap(): Promise<void> {
    console.log('gong connector bootstrap started');
  ...
  async fetchChanges(since: Date): Promise<IEpisode[]> {
    ...
    try {
      ...
      if (connectedAccount?.status !== ConnectorStatus.ACTIVE) {
        console.warn(`gong is not connected for user ${this.config.identifier}. Status: ${connectedAccount?.status}`);
        const linkResponse = await scalekitActions.getAuthorizationLink({
          connectionName: this.name,
          identifier: this.config.identifier!,
        });
        console.warn(`🔗 User must click on this link to authorize gong: ${linkResponse.link}`);
        return [];
      }
      ...
      console.error('Error fetching gong changes via Scalekit:', error);
      throw error;
    }
    return episodes;
  }
