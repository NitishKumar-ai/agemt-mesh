...
export class SalesforceConnector implements Connector {
  ...
  constructor(
    ...
    if (!config.identifier) {
      throw new Error('salesforce connector requires an identifier (User ID) in config to use Scalekit');
    }
  }

  async bootstrap(): Promise<void> {
    console.log('salesforce connector bootstrap started');
  ...
  async fetchChanges(since: Date): Promise<IEpisode[]> {
    ...
    try {
      ...
      if (connectedAccount?.status !== ConnectorStatus.ACTIVE) {
        console.warn(`salesforce is not connected for user ${this.config.identifier}. Status: ${connectedAccount?.status}`);
        const linkResponse = await scalekitActions.getAuthorizationLink({
          connectionName: this.name,
          identifier: this.config.identifier!,
        });
        console.warn(`🔗 User must click on this link to authorize salesforce: ${linkResponse.link}`);
        return [];
      }
      ...
      console.error('Error fetching salesforce changes via Scalekit:', error);
      throw error;
    }
    return episodes;
  }
