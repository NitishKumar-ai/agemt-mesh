...
export class HubspotConnector implements Connector {
  ...
  constructor(
    ...
    if (!config.identifier) {
      throw new Error('hubspot connector requires an identifier (User ID) in config to use Scalekit');
    }
  }

  async bootstrap(): Promise<void> {
    console.log('hubspot connector bootstrap started');
  ...
  async fetchChanges(since: Date): Promise<IEpisode[]> {
    ...
    try {
      ...
      if (connectedAccount?.status !== ConnectorStatus.ACTIVE) {
        console.warn(`hubspot is not connected for user ${this.config.identifier}. Status: ${connectedAccount?.status}`);
        const linkResponse = await scalekitActions.getAuthorizationLink({
          connectionName: this.name,
          identifier: this.config.identifier!,
        });
        console.warn(`🔗 User must click on this link to authorize hubspot: ${linkResponse.link}`);
        return [];
      }
      ...
      console.error('Error fetching hubspot changes via Scalekit:', error);
      throw error;
    }
    return episodes;
  }
