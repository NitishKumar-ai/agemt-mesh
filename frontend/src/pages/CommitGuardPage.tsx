...
import { AlertTriangle, CheckCircle2, ExternalLink, Github, RefreshCw, Shield, XCircle } from "lucide-react";
...
function VerdictBadge({ verdict }: { verdict: string }) {
  if (verdict === "CONFIRMED") return <span className="badge badge--success"><CheckCircle2 size={12} />CONFIRMED</span>;
  if (verdict === "FALSE_POSITIVE") return <span className="badge badge--muted"><XCircle size={12} />FALSE POSITIVE</span>;
    ...
    es.onmessage = (evt) => {
      try {
        ...
        if (data.event_type === "scan_progress") {
          ...
          setProgress({ step: payload.step, progress_pct: payload.progress_pct,
                        current: payload.current, total: payload.total });
    ...
    <div className="page">
      ...
      <form className="scan-form" onSubmit={submit}>
        <div className="scan-input-row">
          ...
          <label className="scan-max-label">
            Max findings
            <select value={maxFindings} onChange={(e) => setMaxFindings(Number(e.target.value))}
                    disabled={scanStatus === "queued" || scanStatus === "running"}>
        ...
        <section className="scan-progress-section">
          <div className="progress-strip">
            {STEPS.slice(1).map((step, i) => (
              <span key={step}
                    className={i < stepIndex - 1 ? "progress-strip--done"
                              : i === stepIndex - 1 ? "progress-strip--active" : ""}>
                <i />{step}
              </span>
            ))}
          </div>
          ...
          {scanStatus === "complete" && (
            <p className="progress-detail">
              {confirmedCount > 0
                ? <><CheckCircle2 size={14} /> {confirmedCount} CONFIRMED finding{confirmedCount > 1 ? "s" : ""} — GitHub issues filed</>
          ...
          {findings.map((f) => (
            <article key={f.id} className={`finding-card finding-card--${f.verdict.toLowerCase()}`}>
              <div className="finding-header">
                ...
                {f.cvss && <span className="finding-cvss" title="AI-estimated, not validated">{f.cvss}</span>}
                {f.cwe && <span className="finding-cwe">{f.cwe}</span>}
              </div>
              <p className="finding-poc">{f.poc_summary}</p>
              {f.github_issue_url && (
                <a className="finding-issue-link" href={f.github_issue_url} target="_blank" rel="noreferrer">
                  <ExternalLink size={13} />View GitHub issue
                </a>
              )}
              {f.verdict === "CONFIRMED" && !f.issue_filed && (
                <span className="badge badge--warning"><AlertTriangle size={12} />Issue filing failed</span>
              )}
              {f.fix_suggestion && (
                <div className="finding-fix">
                  <button type="button" className="bare-toggle"
                          onClick={() => setExpandedFix(expandedFix === f.id ? undefined : f.id)}>
                    <RefreshCw size={13} />{expandedFix === f.id ? "Hide" : "Show"} suggested fix
                  </button>
      ...
      {scanStatus === "complete" && findings.length === 0 && (
        <div className="empty-card">
          <CheckCircle2 size={20} />No high/critical findings confirmed in this repo.
        </div>
      )}
    </div>
  );
