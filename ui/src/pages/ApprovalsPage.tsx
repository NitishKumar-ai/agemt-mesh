...
export function ApprovalsPage({ events }: { events?: any[] }) {
  ...
  const [confirming, setConfirming] = useState<{ id: number; action: 'approve' | 'reject' } | null>(
    null,
  );
    ...
    if (next.has(id)) {
      ...
      if (approval && !approval.history) {
        try {
          ...
          setApprovals((prev) =>
            prev.map((a) => (a.id === id ? { ...a, history: res.history } : a)),
          );
        } catch (e) {
  ...
  const onDecide = async (approval: ApprovalEvent, approved: boolean) => {
    ...
    try {
      ...
      setApprovals((prev) =>
        prev.map((a) =>
          a.id === approval.id ? { ...a, status: approved ? 'approved' : 'rejected' } : a,
        ),
      );
      setConfirming(null);
      ...
      setApprovals((prev) =>
        prev.map((a) => (a.id === approval.id ? { ...a, history: res.history } : a)),
      );
    } catch (e) {
      ...
      {!loading && approvals.length === 0 && (
        <div className="empty-card" style={{ padding: '60px 20px' }}>
          ...
          <p className="body-sm" style={{ color: 'var(--muted)' }}>
            No pending approvals required at this time.
          </p>
        </div>
      )}
            ...
            >
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, flex: 1 }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span className="title-sm" style={{ color: 'var(--ink)' }}>
                        {reason}
                      </span>
                      <span
                        className="badge"
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  {isExpanded ? (
                    <ChevronUp size={20} color="var(--muted-soft)" />
                  ) : (
                    <ChevronDown size={20} color="var(--muted-soft)" />
                  )}
                </div>
              </div>
                ...
                >
                  <div style={{ marginTop: '20px' }}>
                    <h4
                      ...
                      style={{ marginBottom: '12px', color: 'var(--muted)' }}
                    >
                      Action Payload
                    </h4>
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <p style={{ marginTop: '4px' }}>
                            Agent {agentId} requested human sign-off for this action.
                          </p>
                        </div>
                      </div>
                        ...
                        >
                          <div
                            style={{
                              ...
                              borderRadius: '50%',
                              background:
                                h.action === 'approved' ? 'var(--success)' : 'var(--error)',
                              marginTop: '4px',
                            }}
                          />
                          <div style={{ flex: 1 }}>
                            ...
                            <p style={{ marginTop: '4px' }}>
                              By {h.actor}.{' '}
                              {typeof h.payload?.note === 'string' && (
                                <span>Note: "{h.payload.note}"</span>
                              )}
                            </p>
                          </div>
                        </div>
                        ...
                        >
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                              ...
                              <time>
                                {new Date(
                                  new Date(approval.created_at).getTime() + 86400000,
                                ).toLocaleString()}
                              </time>
                            </div>
                            <p style={{ marginTop: '4px' }}>
                              System auto-rejected this request after 24h timeout.
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {confirming?.id === approval.id ? (
                        <div style={{ animation: 'fadeIn 200ms ease' }}>
                          ...
                          <div style={{ marginBottom: '16px' }}>
                            <label
                              className="caption-upper"
                              style={{ display: 'block', marginBottom: '8px', fontSize: '10px' }}
                            >
